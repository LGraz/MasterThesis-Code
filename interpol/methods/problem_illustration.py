...
method_label_args = [
      ("get_smoothing_spline", "smoothing_spline", {"smooth":0.01})
    # , ("get_cubic_spline", "cubic_spline", {})
    , ("get_b_spline", "b_spline", {})
    , ("get_ordinary_kriging", "ordinary_kriging", {"ok_args":{"variogram_model": "gaussian", "variogram_parameters": list(kriging_med_param)}})
    # , ("get_savitzky_golay", "savitzky_golay", {})
    , ("get_fourier", "fourier", {"opt_param":{"p0": [350, 1, 1, 1, 1, 1],
                                                   "bounds": ([50, -1, -5, -5, -5, -5], [500, 2, 5, 5, 5, 5])}})
    , ("get_double_logistic", "double_logistic", {"opt_param":{"p0": [0.2, 0.8, 50, 100, 0.01, -0.01],
...
for pix, ax_ind in zip(pixels_chosen, ax_inds):
    ...
    for method, label, args in method_label_args:
        ...
        pix.plot_step_interpolate(label, label=label)
plt.sca(ax[2,2])
