# from http://www.jtrive.com/loess-nonparametric-scatterplot-smoothing-in-python.html
...
def loess(xvals, yvals, alpha, xx=None, apriori_weights=None, poly_degree=1, robustify=False):
    """
    ...
    """
    if apriori_weights is None:
        apriori_weights = np.ones(len(xvals))
    ...
    if xx is None:
        ...
        xx = np.linspace(start=v_lb, stop=v_ub, num=m)
    v = enumerate(xx, start=1)
    ...
    if robustify == True:
        ...
        while True:
            ...
            for i in robust_est[-1]['loc']:
                ...
                r_i = [(1 - np.abs(j**2))**2 if np.abs(j)
                       < 1 else 0 for j in e2_i]
            ...
            idiffs = \
                np.abs((robust_est[-2]["g"] - robust_est[-1]
                       ["g"]) / robust_est[-2]["g"])

            if ((np.all(idiffs < .005)) or cycle_nbr > 50):
                break
