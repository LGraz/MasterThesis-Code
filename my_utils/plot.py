...
def plot_ndvi_corr_step(pix: pixel.Pixel, name, corr_method_name, corr_response,
    ...
    pix.itpl("ss", itpl.smoothing_spline,
             strategies.identity_no_xtpl, smooth=x_axis)
    rob_args = (itpl.smoothing_spline, strategies.robust_reweighting)
    rob_kwargs = {"y": y, "smooth": x_axis,
    ...
    for case in ["ndvi", "itpl", "itpl_rew", "ndvi_scl", "show_res", "corr", "uncert", "corr_itpl_rew"]:
        ...
        pix.plot_ndvi(colors="scl45",
                      alpha=0 if case not in ["ndvi", "itpl", "itpl_rew"] else 1)
        if case == "ndvi":
        ...
        # NDVI ALL
        alpha_temp = transparancy * \
            1.2 if case not in ["ndvi_scl", "show_res"] else 1
        ...
        if case == "show_res":
            ...
            plt.plot([x_temp, x_temp], [
                pix.ndvi[ind_leave_out], out_of_box_ndvi], c="red")
            plt.savefig('../latex/figures/step_plot/' + name +
                        str(i) + "_" + case+".pdf", bbox_inches='tight')
            continue
