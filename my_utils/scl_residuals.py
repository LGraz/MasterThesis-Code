...
def _calc_res_help(pix, scl_class, itpl_method, **itpl_args):
    method = "scl_45"
    result = []
    ind_train = pix.filter(method)
    getattr(pix, itpl_method)(
        **{**itpl_args, "name": method, "ind_keep": ind_train})
    ind_test = pix.filter("scl" + str(scl_class))
    for i, is_class in enumerate(ind_test):
        if is_class:
            # `das` can be used to find the corresponding row in pix.itpl_df
            das = pix.cov.das.iloc[i]
            das0 = pix.cov.das.iloc[0]
            obs = pix.get_ndvi().iloc[i]
            est = pix.itpl_df[method].iloc[das - das0]
...
def get_residuals(frac, scl_class, itpl_method, itpl_args,
                  itpl_method_args_str, years=None, seed=4321, WW_cereals="cereals", save=True):
    """
    Description
    -----------
    ...
    file_name = "scl_" + species + itpl_method_args_str + "_" + str(scl_class) +\
    ...
    if os.path.isfile(file_path):
        ...
        pixels = data_handle.get_pixels(
            frac, cloudy=True, WW_cereals=WW_cereals, years=years, seed=seed)
        result_df = calc_residuals(
            pixels, scl_class, itpl_method, **itpl_args)
        if save:
